...
public class BD1Loader {
	...
	public static List<BufferedVertices> LoadBD1(String bd1_filename) {
		...
		Map<Integer, String> texture_filenames = manipulator
				.GetTextureFilenames();
		Map<Integer, Integer> texture_handles_map = new HashMap<>();
		for (Map.Entry<Integer, String> entry : texture_filenames.entrySet()) {
		...
		Map<Integer, List<BD1Triangle>> triangles_map = triangulator
				.GetTrianglesMap();
		for (Map.Entry<Integer, List<BD1Triangle>> entry : triangles_map
				.entrySet()) {
			int texture_id = entry.getKey();
			...
			FloatBuffer pos_buffer = Buffers
					.newDirectFloatBuffer(vertex_num * 3);
			FloatBuffer uv_buffer = Buffers
					.newDirectFloatBuffer(vertex_num * 2);
			FloatBuffer norm_buffer = Buffers
					.newDirectFloatBuffer(vertex_num * 3);
